# Puts four plots from the below scripts together for foundation pub- Fescue raw and comparative soil moisture and surface temperature extremes for the facility

source('scripts/M14_package_prep.R')
source('scripts/Churchilletal2022_Figure3_soildata.R')
source('scripts/Churchilletal2022_Figure3_surfTdata.R')

#multipanel
tiff(file = paste("Churchilletal2022_Figure3_",sD,"_",eD,".tiff",sep=""), width =180, height = 240, units = "mm", res = 300)
...
text(x = corners2[2]+corners2[2]/1650, y = mean(corners2[3:4])-mean(corners2[3:4])/1.15, "Irrigation\n(mm)",srt=270,cex=0.7)
text(x = corners2[2]+corners2[2]/3900, y = mean(corners2[3:4])-mean(corners2[3:4])/1.165, "20\n10\n0",las=0,cex=0.8)
...
abline(h=40,lty=2)
legend(legend=c("Maximum Air Temp"),lty=c(1),col=c("black"),lwd=3,cex=1,box.lwd=0,box.col="white",bg="white",x=min(max[[1]]$Date)-7,y=ymax+1)
...
legend(legend=c("Minimum Air Temp"),lty=c(1),col=c("black"),lwd=3,cex=1,box.lwd=0,box.col="white",bg="white",x=min(max[[1]]$Date)-7,y=ymax)
corners = par("usr") #Gets the four corners of plot area (x1, x2, y1, y2)
