...
Irrig2<-subset(Irrig[Irrig$Date>="2018-06-01"&Irrig$Date<="2018-11-30",], Treatment == "Drt")
Irrig2[Irrig2$Irrigation==0.000000,]<-NA
Irrig3<-subset(Irrig[Irrig$Date>="2019-06-01"&Irrig$Date<="2019-11-30",], Treatment == "Drt")
Irrig3[Irrig3$Irrigation==0.000000,]<-NA
...
par(new=TRUE)
with(Irrig3,plot(Irrigation~Date,
                 type="h",
                 col=2,
                 axes=FALSE,
                 xaxt='n',
...
text(x = corners2[2]+corners2[2]/600, y = mean(corners2[3:4])-52, "Irrigation\n(mm)",srt=270,cex=0.6)
text(x = corners2[2]+corners2[2]/1600, y = mean(corners2[3:4])-52, "20\n10\n0",las=0,cex=0.65)
...
corners2 = par("usr") #Gets the four corners of plot area (x1, x2, y1, y2)
par(xpd = TRUE) #Draw outside plot area
text(x = corners2[2]+corners2[2]/600, y = mean(corners2[3:4])-64.5, "Irrigation\n(mm)",srt=270,cex=0.7)
text(x = corners2[2]+corners2[2]/1400, y = mean(corners2[3:4])-64.5, "20\n\n10\n\n0",las=0,cex=0.55)
axis(side=4,at=c(0,10,20),las=2,labels=c("","",""))
