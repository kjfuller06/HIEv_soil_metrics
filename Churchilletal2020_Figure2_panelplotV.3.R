...
#multipanel
tiff(file = paste("FIELD_env_plots_",sD,"_",eD,"V.3.tiff",sep=""), width =1100, height = 1400, units = "px", res = 200)
...
abline(h=40,lty=2)
legend(legend=c("Maximum Air Temp"),lty=c(3),col=c("black"),lwd=1.5,cex=0.5,box.lwd=0,box.col="white",bg="white",x=min(max[[1]]$Date)-14,y=ymax+1)
...
legend(legend=c("Minimum Air Temp"),lty=c(3),col=c("black"),lwd=1.5,cex=0.5,box.lwd=0,box.col="white",bg="white",x=min(max[[1]]$Date)-14,y=ymax)
corners = par("usr") #Gets the four corners of plot area (x1, x2, y1, y2)
